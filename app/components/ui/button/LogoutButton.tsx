...
export default function LogoutButton({session}:{session:any}) {
  ...
  return (
    <div className="w-full md:block md:w-auto">
      <button onMouseEnter={() => setShowTooltip(true)} onMouseLeave={() => setShowTooltip(false)} onClick={() => handleSignOut(session).then(() => signOut())} className="h-full hover:bg-neutral-700 transition-all rounded-md text-xs p-2 right-0" ><FaSignOutAlt/></button> 
